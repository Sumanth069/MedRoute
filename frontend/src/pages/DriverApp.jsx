...
export default function DriverApp() {
  ...
  const activeManifests = manifests.filter(m => m.status === 'pending' || m.status === 'in_transit');
  const deliveredManifests = manifests.filter(m => m.status === 'delivered');

  const renderManifestCard = (m) => {
    return (
      <div key={m.id} className="driver-manifest-card" style={{ marginBottom: '14px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          <span className="manifest-status-badge pending" style={{ 
            color: m.status === 'delivered' ? (m.has_discrepancy ? '#ef4444' : '#10b981') : m.status === 'in_transit' ? '#2563eb' : '#d97706',
            backgroundColor: m.status === 'delivered' ? (m.has_discrepancy ? 'rgba(239, 68, 68, 0.08)' : 'rgba(16, 185, 129, 0.08)') : m.status === 'in_transit' ? 'rgba(37, 99, 235, 0.08)' : 'rgba(217, 119, 6, 0.08)',
            border: m.status === 'delivered' ? (m.has_discrepancy ? '1px solid rgba(239, 68, 68, 0.15)' : '1px solid rgba(16, 185, 129, 0.15)') : m.status === 'in_transit' ? '1px solid rgba(37, 99, 235, 0.15)' : '1px solid rgba(217, 119, 6, 0.15)'
          }}>
        ...
        <div style={{ marginBottom: '12px' }}>
          ...
          <h4 style={{ fontSize: '16px', color: 'var(--text-bright)', marginTop: '8px', fontWeight: '700' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', fontSize: '13.5px', marginBottom: '14px' }}>
          ...
          <div style={{ paddingLeft: '8px', borderLeft: '2px dashed rgba(0,0,0,0.06)', marginLeft: '7px', height: '12px' }}></div>
        ...
        <div style={{ display: 'flex', gap: '12px', fontSize: '12.5px', color: 'var(--text-muted)', marginBottom: '16px', borderTop: '1px solid rgba(0,0,0,0.04)', paddingTop: '10px' }}>
  ...
  return (
    <div className="driver-container">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
        <div>
          <h2 style={{ fontSize: '20px', color: 'var(--text-bright)', margin: 0, fontWeight: '800' }}>Driver Manifests</h2>
        ...
        <div style={{ display: 'flex', flex: '1', flexDirection: 'column', gap: '20px' }}>
          ...
          <div>
            <h3 style={{ 
              ...
              <Truck size={14} />
              Active Shipments ({activeManifests.length})
            </h3>
            {activeManifests.length === 0 ? (
              <div className="glass-panel" style={{ textAlign: 'center', padding: '2rem 1.25rem', color: 'var(--text-muted)' }}>
                <p style={{ fontSize: '12.5px', margin: 0 }}>No active shipments assigned. Check completed tab below.</p>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                {activeManifests.map(renderManifestCard)}
              </div>
            )}
          </div>
          ...
          <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '20px', marginTop: '10px' }}>
            <h3 style={{ 
              ...
              <CheckCircle size={14} />
              Completed Deliveries ({deliveredManifests.length})
            </h3>
            {deliveredManifests.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '1.5rem 1.25rem', color: 'var(--text-muted)', border: '1px dashed var(--border-color)', borderRadius: '12px' }}>
                <p style={{ fontSize: '12.5px', margin: 0 }}>No completed shipments recorded yet.</p>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', opacity: 0.88 }}>
                {deliveredManifests.map(renderManifestCard)}
              </div>
            )}
          </div>
